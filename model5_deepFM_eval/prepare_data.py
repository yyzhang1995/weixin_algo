...
def prepare_data(params):
    ...
    for action in tqdm(ACTION_LIST):
        ...
        df_neg_date14 = df_neg[df_neg['date_'] == 14]
        df_neg_before = df_neg[df_neg['date_'] <= 13]
        df_neg_before = df_neg_before.sample(frac=1.0 / ACTION_SAMPLE_RATE[action], random_state=42, replace=False)
        df_all = pd.concat([df_neg_date14, df_neg_before, tmp[tmp[action] == 1]])
